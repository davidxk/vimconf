...
def findSections(filename, lines):
    ...
    for i, line in enumerate(lines):
        ...
        if isSection:
            parent = findParent(previousSections, level)
            s = Section(level, name, content, lineNumber, filename, parent)
            previousSections[level] = s
            previousSections[level+1:] = [None] * (6-level)
            sections.append(s)
